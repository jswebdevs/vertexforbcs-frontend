...
const GallerySettings = () => {
  ...
  const handleAddSlider = () => {
    if (newsrc && newTitle) {
      ...
      console.log("File name:", newsrc.name);
      formData.forEach((value, key) => {
        console.log(`${key}: ${value}`);
      });

      fetch("https://vertexforbcs.com/vartexforbcs/web/gallery/create", {
        method: "POST",
  ...
  const handleDeletesrc = () => {
    fetch(
      `https://vertexforbcs.com/vartexforbcs/web/gallery/delete/${srcToDelete.id}`,
      {
        method: "DELETE",
      }
    )
    ...
    <div className="flex justify-between p-4">
      ...
      <div className="w-full">
        <div className="flex justify-between">
          <div>
            <h2 className="text-2xl mb-4">Gallery Images</h2>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {sliders.length === 0 ? (
            <p>No image available. Please add a new image.</p>
          ) : (
            sliders.map((slider) => (
              <div
                key={slider.id}
      ...
      {isAddPopupOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div ref={addPopupRef} className="bg-white p-4 rounded shadow-lg">
            <h3 className="text-lg font-semibold mb-2 text-black">Add New Image</h3>
            <input
              type="file"
      ...
      {isDeletePopupOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div ref={deletePopupRef} className="bg-white p-4 rounded shadow-lg">
            ...
            <img src={srcToDelete.src} alt={srcToDelete.alt} className="mb-2 max-h-[400px]" />
            <div className="flex justify-end">
              <button
